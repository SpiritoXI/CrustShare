...
import { FileIcon, X, CheckCircle, AlertCircle, Globe, HardDrive } from 'lucide-react';
import useStore from '@/store/useStore';
import { toast } from 'sonner';

interface CrustFileUploadProps {
  file: File;
  onClose: () => void;
}

export default function CrustFileUpload({ file, onClose }: CrustFileUploadProps) {
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="crystal-dialog sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {status === 'uploading' ? '正在上传到去中心化存储网络...' : null}
